...
private const val BASE_STYLE_51 = "https://tiles.openfreemap.org/styles/liberty"

internal data class MapLayers51(
...
private data class MapMarker51(val point: GeoPoint, val label: String)
...
fun NativeTripMap51(
    ...
    val controller = remember(context) { NativeMapController51(context) }
    ...
    AndroidView(
        ...
        update = { controller.update(snapshot, previewRoute, parkings, layers) },
        modifier = modifier
    )
}

private class NativeMapController51(context: Context) {
    val mapView: MapView
    ...
    private var latestLayers = MapLayers51()
    ...
    init {
        ...
        mapView.getMapAsync { readyMap ->
            ...
            readyMap.setStyle(Style.Builder().fromUri(BASE_STYLE_51)) { readyStyle ->
    ...
    private fun render() {
        ...
        if (route.size >= 2) {
            for (i in 0 until route.lastIndex) {
                val level = if (latestLayers.traffic) {
                    congestion.getOrNull(i)?.takeIf { byLevel.containsKey(it) } ?: "unknown"
                } else {
                    "unknown"
                }
        ...
        upsertLine(style, "route51-low", byLevel.getValue("low"), Color.rgb(24, 121, 78), 6f)
        upsertLine(style, "route51-moderate", byLevel.getValue("moderate"), Color.rgb(210, 158, 0), 6f)
        upsertLine(style, "route51-heavy", byLevel.getValue("heavy"), Color.rgb(229, 107, 53), 6f)
        upsertLine(style, "route51-severe", byLevel.getValue("severe"), Color.rgb(180, 35, 24), 7f)
        upsertLine(style, "route51-unknown", byLevel.getValue("unknown"), Color.rgb(23, 107, 135), 5f)

        val current = latestSnapshot.lat?.let { lat ->
            latestSnapshot.lon?.let { lon -> GeoPoint(lat, lon, "Dein Standort", "current") }
        }
        val tolls = if (latestLayers.tolls) latestSnapshot.tolls.ifEmpty { latestPreview?.tolls.orEmpty() } else emptyList()
        val fallbackFuels = if (latestLayers.fuel) TripConfig.fuelStops(latestSnapshot.stage) else emptyList()
        val recommended = if (latestLayers.fuel) latestSnapshot.fuelSuggestion else null
        val parkings = if (latestLayers.parking) latestParkings else emptyList()

        upsertLabeledPoints(
            style, "point51-current",
            current?.let { listOf(MapMarker51(it, "Du")) }.orEmpty(),
            Color.rgb(23, 107, 135), 8f
        )
        upsertLabeledPoints(
            style, "point51-tolls",
            tolls.map { MapMarker51(it.point, "Maut · ${it.name.substringBefore(" · ")}") },
            Color.rgb(183, 121, 0), 7f
        )
        upsertLabeledPoints(
            style, "point51-fallback-fuels",
            fallbackFuels.map { MapMarker51(it, "Tank · ${it.name}") },
            Color.rgb(126, 87, 194), 7f
        )
        upsertLabeledPoints(
            style, "point51-recommended-fuel",
            recommended?.let { fuel ->
                val price = fuel.pricePerLitre?.let { String.format(Locale.GERMANY, "%.3f €/l", it) }
                    ?: "Preis wird geladen"
                listOf(MapMarker51(fuel.point, "$price\n${fuel.name}"))
            }.orEmpty(),
        ...
        upsertLabeledPoints(
            style, "point51-parkings",
            parkings.map { saved -> MapMarker51(saved.spot.point, "P · ${saved.spot.name}") },
        ...
        upsertLabeledPoints(
            style, "point51-start",
            listOf(MapMarker51(origin, "Start · ${origin.name}")),
            Color.rgb(24, 38, 63), 8f
        )
        upsertLabeledPoints(
            style, "point51-destination",
            listOf(MapMarker51(destination, "Ziel · ${destination.name}")),
